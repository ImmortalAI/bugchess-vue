...
import { BarChart2, Languages, LogIn, LogOut, Moon, Settings, Sun } from '@lucide/vue';
  ...
  <DropdownMenu>
    <DropdownMenuTrigger as-child>
      <button
        ...
        :class="{ grayscale: props.grayscale }" v-html="avatarSvg" />
    </DropdownMenuTrigger>
    ...
    <DropdownMenuContent align="end" class="w-48">
      ...
      <template v-if="auth.isAuthenticated">
        <DropdownMenuSeparator />
        <DropdownMenuItem @click="router.push('/stats')">
      ...
      <DropdownMenuItem @click="toggleDark()">
        <div class="relative mr-2 size-4 shrink-0">
          <Sun class="absolute size-4 transition-all duration-300"
            :class="dark ? 'opacity-0 rotate-90 scale-0' : 'opacity-100 rotate-0 scale-100'" />
          <Moon class="absolute size-4 transition-all duration-300"

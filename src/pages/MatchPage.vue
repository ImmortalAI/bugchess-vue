...
<template>
  ...
  <div v-if="isMobile" class="w-full h-full flex flex-col gap-2 py-2 overflow-y-auto"
    ...
    <div ref="mobileBoardArea" class="grid w-full">
      <!-- Main board -->
      <div class="col-start-1 row-start-1 flex flex-col gap-1"
        :class="{ 'invisible pointer-events-none': boardReversed }">
        <div class="flex justify-between items-center gap-2 px-2">
          <ChessPockets orientation="horizontal" :board-size="mobileBoardSize" :color="opponentColor"
        ...
        <ChessBoard :size="mobileBoardSize" :config="game.mainBoardState" :is-promoting="game.isPromoting"
          :promotion-color="game.promotionColor" :promotion-file="game.promotionFile" @ready="game.registerMainBoard"
          @promotion-select="game.promote" />
        <div class="flex justify-between items-center gap-2 px-2">
          <ChessPockets orientation="horizontal" :board-size="mobileBoardSize" :color="myColor"
            :data="mainMyPocket ?? EMPTY_POCKET" interactive @piece-mousedown="game.dragMainPocketPiece" />
      ...
      <div class="col-start-1 row-start-1 flex flex-col gap-1"
        ...
        <ChessBoard :size="mobileBoardSize" :config="game.mateBoardState" @ready="game.registerMateBoard" />
        <div class="flex justify-between items-center gap-2 px-2">
          <ChessPockets orientation="horizontal" :board-size="mobileBoardSize" :color="opponentColor"
  ...
  <div v-else class="w-full h-full flex justify-center items-center gap-4" :class="{ invisible: !session.initialized }">
    ...
    <div class="flex gap-2 items-center">
      ...
      <div class="flex">
        <div class="relative">
          <ChessBoard :size="boardSize" :config="game.mainBoardState" :is-promoting="game.isPromoting"
            :promotion-color="game.promotionColor" :promotion-file="game.promotionFile" @ready="game.registerMainBoard"
            @promotion-select="game.promote" />
          <ChessBoardResizer v-if="!game.isPromoting" @resize="resizeBy" />
        </div>
        <ChessPockets :board-size="boardSize" :color="myColor" :data="mainMyPocket ?? EMPTY_POCKET"
          :data-opponent="mainOpponentPocket" interactive @piece-mousedown="game.dragMainPocketPiece" />
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2">
      <div class="flex">
        <ChessBoard :size="MATE_BOARD_SIZE" :config="game.mateBoardState" @ready="game.registerMateBoard" />
        <ChessPockets :board-size="MATE_BOARD_SIZE" :color="opponentColor" :data="matePocketPartner ?? EMPTY_POCKET"
